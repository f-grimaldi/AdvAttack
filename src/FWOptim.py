...
class FWOptim():
    ...
    def run(self, x, n_gradient, delta, beta, gamma, epsilon,
            L_type=-1, batch_size = -1,
        ...
        n_gradient:     (int)               Number of normal vector to generate at every step
        ...
        # 3. Compute m0
        m = self.gradient(x, n_gradient, delta, verbose)

        # 4. Main cycle
        for ep in tqdm(range(max_steps), disable=tqdm_disabled):

            # 4.1 Compute new values
            x, m = self.step(x.view(*list(self.dim)), m, n_gradient, delta, beta, gamma, verbose)
            ...
            if self.L_type == -1:
                ...
                condition3 = torch.norm(x.view(*list(self.dim) - self.x_ori)) < epsilon
        ...
        return x.view(*list(self.dim)), losses, outs
    ...
    def gradient(self, x, b, delta, verbose=0):
        ...
        for i in range(n_batches):
            ...
            # 4. Get loss
            first_term = self.loss(out_max).detach().view(-1, 1).detach()                               # Dim bs, 1
            second_term = self.loss(out_min).detach().view(-1, 1).detach()                              # Dim bs, 1
