...
class HDZOOptim(object):
    ...
    Perform a zero-order optimization
    """
    def run(self, x, c, learning_rate=1e-2, n_gradient=128, h=1e-4, beta_1=0.9,
            beta_2=0.999, solver="adam", verbose=False, max_steps=10000, batch_size=-1,
            C=(0, 1), stop_criterion=True, tqdm_disabled=False, additional_out=False,
            reset_adam=True, hierarchical=True, importance_sampling=False):
        """
        Args:
            Name                    Type                Description
            x:                      (torch.tensor)      The variable of our optimization problem. Should be a 3D tensor (img)
            ...
            tqdm_disabled:          (bool)              Disable the tqdm bar. Default is False
            additional_out:         (bool)              Return also all the x. Default is False
            reset_adam:             (bool)              If true reset ADAM parameters after the first valid image has been found
            hierarchical:           (bool)              If true use hierarchical approach
            importance_sampling:    (bool)              If true use probability distribution to sample pixels
        return:
            x:                      (torch.tensor)      Final image
            losses:                 (list)              Loss at each step
            outs:                   (list)              Adversarial example at each step
        ...
        if hierarchical:
            if x_dim[1] < 32:
                raise RuntimeError("Can't use hierarchical attack on images with dimension lower than 32x32")
            curr_dim = 32
            y = torch.zeros(1, 3, curr_dim, curr_dim).to(self.device)

        # Importance sampling
        probs = np.ones(total_dim)
        ...
        for iteration in tqdm(range(max_steps), disable=tqdm_disabled):
            ...
            # Hierarchical attack
            if hierarchical and iteration == 100:
                if x_dim[1] >= 64:
                    curr_dim = 64
                    torch.nn.functional.interpolate(y, (curr_dim, curr_dim), mode='bilinear')
                else:
                    curr_dim = x_dim[1]
                    torch.nn.functional.interpolate(y, (curr_dim, curr_dim), mode='bilinear')
            elif hierarchical and iteration == 200 and x_dim[0] >= 64:
                curr_dim = x_dim[1]
                torch.nn.functional.interpolate(y, (curr_dim, curr_dim), mode='bilinear')

            # 2.1 Call the step
            y = self.step(x_0, y, probs, c, learning_rate, n_gradient, h, beta_1, beta_2,
                          solver, x_dim, total_dim, batch_size, C, verbose, importance_sampling)

            # 2.2 Compute new loss and store current info
            curr_imm = self.project_boundaries(x_0 + y, C)
            out = self.model(x.view(1, x_dim[0], x_dim[1], x_dim[2]))
            curr_loss_st = self.loss(out)
            curr_l2 = torch.norm(y).detach()
            curr_loss = curr_l2 + c * curr_loss_st
            ...
            if found:
                ...
                if not losses_st or losses_st[-1] != 0:
                    if not tqdm_disabled:
                        print(
                            "First valid image found at iteration {} with l2-distance = {}".format(iteration, curr_l2))
                    best_l2 = curr_l2
                    best_image = (x_0 + y).clone()
                    best_loss = curr_loss
                    # Reset ADAM state
                    if reset_adam:
                        self.M = torch.zeros(x.shape).to(self.device)
            ...
            if verbose:
                print('-----------STEP {}-----------'.format(iteration))
                print('Iteration: {}'.format(iteration))
                print('Shape of x: {}'.format(curr_imm.shape))
                print('Second term loss:    {}'.format(curr_loss_st.cpu().item()))
                print('L2 distance: {}'.format(curr_l2))
                print("Adversarial Loss:    {}".format(curr_loss))

            # 2.7 Reshape y
            y = y.view(1, 3, x_dim[1], x_dim[2])
            # Update probability
            if importance_sampling and iteration % 20 == 0:
                probs = max_pooling.forward(y).view(-1)
                probs = probs/torch.sum(probs)
            torch.nn.functional.interpolate(y, (curr_dim, curr_dim), mode='bilinear')

            # 2.8 Evaluate stop criterion
            if stop_criterion:
                if len(losses) > 20:
                    if curr_loss_st == 0 and l2_dists[-1] == l2_dists[-20]:
                        break
    ...
    Do an optimization step
    """
    def step(self, x_0, y, probs, c, learning_rate, n_gradient, h, beta_1, beta_2,
             solver, x_dim, total_dim, batch_size, C, verbose, importance_sampling):
        """
        Args:
            Name                    Type                Description
            x_0:                    (torch.tensor)      Linearized original image
            y:                      (torch.tensor)      Added noise
            probs:                  (torch.tensor)      Probability used to sample pixels
            c:                      (float)             Regularization parameter
            learning_rate:          (float)             Learning rate
            n_gradient:             (int)               Coordinates we simultaneously optimize
            h:                      (float)             Gradient estimation accuracy O(h^2)
        ...
        if n_gradient > total_dim:
            raise ValueError("Batch size must be lower than the total dimension")
        if importance_sampling:
            indices = np.random.choice(total_dim, n_gradient, replace=False, p=probs.view(-1).cpu())
        else:
            indices = np.random.choice(total_dim, n_gradient, replace=False)
        e_matrix = torch.zeros(n_gradient, total_dim).to(self.device)
        for n, i in enumerate(indices):
        ...
        y = torch.nn.functional.interpolate(y, (x_dim[1], x_dim[2]), mode='bilinear').view(-1, 1)

        # 2. Optimizers
        if solver == "adam":
            # 2.1 Gradient approximation
            g_hat = self.compute_gradient(x_0, y, e_matrix, c, n_gradient, h, solver, x_dim, total_dim, batch_size,
                                          verbose).view(-1, 1)
            ...
            if verbose > 1:
                print('-------------ADAM------------')
                print('The input x has shape:\t\t{}'.format(y.shape))
                print('Chosen indices are: {}\n'.format(indices))
                print('T = {}'.format(self.T))
                print('M = {}'.format(self.M))
                print('v = {}'.format(self.v))
            ...
            # 2.1 Gradient and Hessian approximation
            g_hat, h_hat = self.compute_gradient(x_0, e_matrix, c, n_gradient, h, solver, x_dim, total_dim, batch_size,
                                                 verbose, y)
            g_hat = g_hat.view(-1, 1)
            h_hat = h_hat.view(-1, 1)

            # 2.2 Update
            delta = torch.zeros(y.shape).to(self.device)
            h_hat[h_hat <= 0] = 1
            delta[indices] = -learning_rate * (g_hat / h_hat)
            y = y + delta.view(-1, 1)
    ...
    Compute Gradient and Hessian
    """
    def compute_gradient(self, x_0, y, e_matrix, c, n_gradient, h, solver, x_dim, total_dim, batch_size, verbose):
        """
        Args:
            x_0:                    (torch.tensor)      Linearized original image
            y:                      (torch.tensor)      Added noise
        ...
        for i in range(n_batches):
            # 2.1 Expand image along one direction
            x_expanded = (x_0 + y).view(-1).expand(batch_size, total_dim).to(self.device)
            x_0_expanded = x_0.view(-1).expand(batch_size, total_dim).to(self.device)
            ...
            if solver == 'newton':
                ...
                loss2_added = self.loss(self.model(x_expanded.view(batch_size, *list(x_dim))))
                additional_term = loss1_added + (c * loss2_added)
                h_hat[batch_size * i:batch_size * (i + 1), :] = (first_term.view(-1, 1) + second_term.view(-1,
                                                                                                           1) - 2 * additional_term.view(
                    -1, 1)) / (h ** 2)
...
"""
Computes Max-Pooling used for importance sampling
"""
class MaxPooling(nn.Module):
    ...
    def forward(self, x):
        return torch.max_pool2d(x, kernel_size=7, stride=1, padding=3)
